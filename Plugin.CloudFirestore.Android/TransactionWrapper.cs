...
namespace Plugin.CloudFirestore
{
    public class TransactionWrapper : ITransaction
    {
        ...
        {
            var wrapper = (DocumentReferenceWrapper)document;
            _transaction.Set((DocumentReference)wrapper, documentData.ToNativeFieldValue());
        }

        public void SetData(IDocumentReference document, object documentData, params string[] mergeFields)
        {
            var wrapper = (DocumentReferenceWrapper)document;
            _transaction.Set((DocumentReference)wrapper, documentData.ToNativeFieldValue(), SetOptions.MergeFields(mergeFields));
        }

        public void SetData(IDocumentReference document, object documentData, params FieldPath[] mergeFields)
        {
            var wrapper = (DocumentReferenceWrapper)document;
            _transaction.Set((DocumentReference)wrapper, documentData.ToNativeFieldValue(), SetOptions.MergeFieldPaths(new JavaList<Firebase.Firestore.FieldPath>(mergeFields.Select(x => x.ToNative()))));
        }
        ...
        public void SetData(IDocumentReference document, object documentData, bool merge)
        {
            ...
            var wrapper = (DocumentReferenceWrapper)document;
            _transaction.Set((DocumentReference)wrapper, documentData.ToNativeFieldValue(), SetOptions.Merge());
        }
